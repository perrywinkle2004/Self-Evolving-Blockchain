...
with tab2:
    ...
    if not tr or not tr.records:
        ...
        # Download
        with st.expander("💾 Export Analytics Data (JSON)/(CSV)", expanded=False):
            payload = {
                "records": records,
            ...
            def convert_to_csv(data):
                """Helper function to convert JSON/dict into CSV format using pandas DataFrame."""
                if isinstance(data, dict):
                    dfs = []
                    for k, v in data.items():
                        if isinstance(v, list) and len(v) > 0 and isinstance(v[0], dict):
                            # Flatten nested elements; prefix avoids column name collisions
                            prefix = "" if k == "records" else f"{k}_"
                            dfs.append(pd.json_normalize(v).add_prefix(prefix))
                    if dfs:
            ...
            st.download_button(
                "Download blockchain_data.csv",
                data=convert_to_csv(payload),
                file_name="intellichain_data.csv",
                mime="text/csv",
            )
